...
"""
function sgd!(net::Network{L,Lm1}, train_data, i::Int;
              epochs::Int = 60, mini_batch_size::Int = 10,
              η::Real = 0.01, test_data = nothing) where {L,Lm1}
    @assert epochs > 0
    ...
    for j = 1:epochs
        ...
        for k = mini_batch_size : mini_batch_size : length(train_data)
            ...
            update_mini_batch!(net, a, z, ∇b, ∇w, δ, mini_batch, η, i)
        end
        if test_data ≠ nothing
            res = evaluate(net, test_data, i)
...
function update_mini_batch!(net::Network{L,Lm1},
                            ...
                            mini_batch, η::Real, i::Int) where {L,Lm1}
    @assert 0 < η < Inf
    check_sizes(net, a, z, ∇b, ∇w)
    for v in ∇b v .= 0 end
    for v in ∇w v .= 0 end
    for (input, output) in mini_batch
        backprop!(a, z, ∇b, ∇w, δ, net, input, output, i)
    end
    for l = 1:L-1
...
function backprop!(a::NTuple{L,Vector{Float64}},
                   ...
                   net::Network{L,Lm1},
                   input::Vector{<:Real},
                   output::Vector{<:Real}, i::Int) where {L,Lm1}
    check_sizes(net, a, z, ∇b, ∇w)
    @assert length.(δ) == length.(net.b)
    a[1] .= input
    feedforward!(a, z, net,i)
    δ[L-1] .= cost_prime.(a[L], output) .* act_fn_prime.(z[L-1],i)
    ∇b[L-1] .+= δ[L-1]
    ∇w[L-1] .+= δ[L-1] .* a[L-1]'
    for l = L-2 : -1 : 1
        mul!(δ[l], net.w[l+1]', δ[l+1])
        δ[l] .*= act_fn_prime.(z[l],i)
        ∇b[l] .+= δ[l]
        ∇w[l] .+= δ[l] .* a[l]'
    end
end
