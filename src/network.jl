...
Base.size(net::Network, l) = size(net)[l] #ODO: I don't know if this is needed
...
function feedforward!(a::NTuple{L, Vector{Float64}},
                      ...
                      net::Network{L,Lm1},i::Int) where {L,Lm1}
    check_sizes(net, a, z)
    for l = 1:L-1
        mul!(z[l], net.w[l], a[l])
        z[l] .+= net.b[l]
...
feedforward!(a::NTuple{L,Vector{Float64}}, net::Network{L}, i::Int) where {L} =
    feedforward!(a, a[2:end], net, i)
...
"""
function feedforward(input::Vector{Float64}, net::Network{L}, i::Int) where {L}
...
"""
function evaluate(net::Network, test_data, i::Int)
